...
const Sub = () => {
  ...
  const getfiles = async () => {
    try {
      const response = await fetch(`https://studify-backend.onrender.com/uploadfiles/getfiles/${subject}`, {
        method: "GET",
        headers: {
  ...
  const handlefav=async (fileId)=>{
    try{
      const userId=sessionStorage.getItem('userId');
      const response = await fetch(`https://studify-backend.onrender.com/uploadfiles/favourites/${fileId}`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${sessionStorage.getItem("token")}`,
          "Content-Type": "application/json",
